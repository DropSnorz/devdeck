import { describe, expect, it } from 'vitest'
import { act, renderHook } from '@testing-library/react'
import type { DashboardWidgetInstance } from '@/types/layout'
import { encodeLayout } from './layoutCodec'
import { useShareLinkBootstrap } from './useShareLinkBootstrap'

const WIDGETS: DashboardWidgetInstance[] = [
  { instanceId: 'a', widgetId: 'base64', x: 0, y: 0, w: 2, h: 2 },
]

describe('useShareLinkBootstrap', () => {
  it('reads a pending layout from the URL fragment, and clears the fragment once read', () => {
    window.location.hash = `layout=${encodeLayout(WIDGETS)}`

    const { result } = renderHook(() => useShareLinkBootstrap())

    expect(result.current.pendingLayout?.widgets).toEqual(WIDGETS)
    // Never left sitting in the URL — matches the query-param behavior this
    // replaces, just off window.location.hash instead of .search now.
    expect(window.location.hash).toBe('')
  })

  it('dismiss() clears the pending layout', () => {
    window.location.hash = `layout=${encodeLayout(WIDGETS)}`
    const { result } = renderHook(() => useShareLinkBootstrap())

    act(() => result.current.dismiss())

    expect(result.current.pendingLayout).toBeNull()
    expect(window.location.hash).toBe('')
  })

  it('leaves pendingLayout null when there is no fragment at all', () => {
    window.location.hash = ''

    const { result } = renderHook(() => useShareLinkBootstrap())

    expect(result.current.pendingLayout).toBeNull()
    expect(result.current.decodeError).toBeNull()
  })

  it('surfaces a decode error for a corrupted fragment, without throwing', () => {
    window.location.hash = 'layout=%%%not-valid%%%'

    const { result } = renderHook(() => useShareLinkBootstrap())

    expect(result.current.pendingLayout).toBeNull()
    expect(result.current.decodeError).toBeTruthy()
    expect(window.location.hash).toBe('')
  })
})
